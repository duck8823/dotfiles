---
description: Claude / Antigravity / Codex を安全な headless research partner として並列実行し、失敗理由も記録する
...
Claude / Antigravity / Codex の headless 調査を「失敗して止まる」ではなく、preflight・安全境界・fallback・失敗理由記録つきで回す。

## 原則

- デフォルトは **auto mode**。git repository では sanitized workspace context packet を作り、Claude / Antigravity / Codex に同一 packet を渡す。
...
- Antigravity は per-run の空ディレクトリから sandbox-first の `agy --print --sandbox` で実行し、repo 直接読みには戻さない。sandbox が host CLI の認証状態だけを隠して `auth_prompt` になった場合は、同一 engine / 同一 prompt / empty cwd / `NO_BROWSER=true` / no `--add-dir` / no `--sandbox` で 1 回だけ authenticated transport retry する。
- Claude が policy / approval reviewer に拒否された場合は、設定を弱めず status に記録する。

## 実行

```bash
~/.local/bin/multi-ai-research.sh --topic "$ARGUMENTS" --mode auto --engines claude,antigravity,codex
```
...
ユーザーが現在ターンで明示した repo と無関係な public/general Web 調査だけを行う場合は、local files / source / workspace packet を渡さずに general mode で実行する。

```bash
~/.local/bin/multi-ai-research.sh --topic "$ARGUMENTS" --mode general --engines claude,antigravity,codex
```

`~/.local/bin/multi-ai-research.sh` がない場合は、dotfiles root から:

```bash
./scripts/multi-ai-research.sh --topic "$ARGUMENTS" --mode auto --engines claude,antigravity,codex
```
...
- `auth_prompt` / login 失敗: Antigravity sandbox 起因の auth-only failure は同一 engine の authenticated transport retry を 1 回だけ試す。retry でも失敗、または他 engine の auth prompt はブラウザを開かず停止。**fallback せずユーザーに認証修正を依頼**（暗黙の engine 代替は設定不備を隠す）。
...
- `process_oom`: 外部 CLI が OOM。空の per-run cwd で実行し、必要なら packet 上限を下げる。
