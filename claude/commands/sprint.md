...
### 1-4. 設計結果の Issue コメント投稿 & 不明点の一括確認

各イシューの設計結果を GitHub Issue にコメント投稿して永続化する。不明点がある場合はユーザーに一括確認する。
...
# 不明点をエンドユーザー体験に関わるものと技術的なものに分類して処理
# - エンドユーザー体験（UI/UX・機能仕様・画面遷移・文言など）→ ユーザーに確認
# - 技術的な判断（ライブラリ選定・アーキテクチャ・構成・命名など）→ 選択肢とトレードオフを提示して自律決定し、決定内容を Issue コメントに記録する
if [ -n "$QUESTIONS" ]; then
