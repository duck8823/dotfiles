...
4. Structure-Behavior drift のリスク（新しい概念・責務・境界/IF・振る舞いテストが必要か）
5. 実装を Claude foreground に残すべきかどうか
...
4. Structure-Behavior Design Note（概念モデル・責務表・境界/IF・振る舞いテスト）が必要か
5. Codex worker に向くか、Claude foreground に残すべきか
...
### セキュリティ・エッジケース
### validation plan
### Structure-Behavior Design Note 要否
### 実装担当の推奨（Claude / Codex と理由）
### 不明点・要確認事項（なければ「なし」）
PROMPT_EOF
...
for ISSUE_NUMBER in "${ISSUES[@]}"; do
  tmux wait-for ${PROJECT}-issue${ISSUE_NUMBER}-antigravity-scout-done
...
for ISSUE_NUMBER in "${ISSUES[@]}"; do
  ANTIGRAVITY_RESULT=$(cat /tmp/${PROJECT}-issue${ISSUE_NUMBER}-antigravity-scout-result.md 2>/dev/null || echo "（Antigravity scout 失敗）")
...
| Antigravity が「diff 外影響が広い」と判断した | **Claude** |
| Structure-Behavior risk が High | **Claude** |
...
- `main` から新しいブランチを作成（CLAUDE.md のブランチ命名規則に従う。チケット番号はブランチ名に含めない）
- scout コメントを参照しつつ関連ファイルを読んで既存パターンを確認する
- Medium / High risk では `structure-behavior-design` の Design Note を `.ai/spec/<issue>.md` に残す
...
- Antigravity scout が指摘した docs / config / l10n drift を取りこぼさない

#### 2-2b: Codex が実装する場合

**実装依頼プロンプト生成（Traceary / git / PR / Issue からの context-resume 形式）:**
...
ANTIGRAVITY_RESULT=$(cat /tmp/${PROJECT}-issue${ISSUE_NUMBER}-antigravity-scout-result.md 2>/dev/null || echo "（Antigravity scout なし）")
...
## Constraints
- ブランチ: CLAUDE.md のブランチ命名規則に従う（チケット番号はブランチ名に含めない）
- コミットメッセージは日本語で記述すること
- 既存の命名規約・コード規約（CLAUDE.md 参照）に従うこと
- main への直接コミット禁止

## Antigravity scout（repo-wide / drift）
${ANTIGRAVITY_RESULT}
...
**実装完了後の確認（current orchestrator が担当）:**
1. `test_command` / `analyze_command` をローカルで再実行して結果を確認
2. 失敗があれば current orchestrator がフォローアップ修正
3. コミット分割を整理する（論理的な単位になっているか確認）
4. Antigravity scout の drift 指摘が反映済みか確認する
5. ドラフト PR 作成 → 2-3 へ

**フォールバック:**
- EXIT_CODE 非ゼロ → 1回リトライ（同じコマンドを再実行）
- 2回目も失敗 → current orchestrator が直接実装または Claude specialist に切り替え
...
**Claude が実装した場合（policy scout + Codex verifier + integrator）:**
| # | AI | 実行方法 | 役割 |
|---|-----|---------|------|
| 1 | Antigravity CLI | tmux + local policy に従う `agy` | 1st pass（設計・一貫性・diff 外影響） |
...
| 3 | Claude/Codex | メインセッション | 統合レビュー（マージ判断） |
...
- **Antigravity は local policy に従う scout / critic / optional worker** として扱う。無効なら skip を記録する
