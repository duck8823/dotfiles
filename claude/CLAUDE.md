...
| プレフィックス | 用途 | 例 |
|---|---|---|
| `feature/` | 機能追加 | `feature/slack-notification` |
| `fix/` | ユーザー影響がある（本番リリース済み）機能の修正 | `fix/push-notification-payload` |
| `maintenance/` | リファクタ、ライブラリ更新、リリース前の機能修正など | `maintenance/migrate-release-drafter-v7` |
...
- チケット番号がある場合: `[チケット番号] 日本語タイトル`（例: `[YUGUN-1234] プッシュ通知ペイロードを修正`）
- チケット番号がない場合: `[MAINTENANCE] 日本語タイトル` など
