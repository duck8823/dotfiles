...
- 既存のデザインパターン・コンポーネントに合わせる。不明な点は実装前に質問する
...
各プロジェクトの CLAUDE.md に `source_dirs`, `source_extensions`, `source_exclude`, `test_command`, `analyze_command` を記載。generated なコードの除外方針は dotfiles 側のレビュー原則で扱い、ignore / exclude の具体管理は各リポジトリ側で決める。
