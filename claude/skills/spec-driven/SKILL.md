...
非自明な変更では `structure-behavior-design` skill の観点を仕様に統合し、要求から即実装へ飛ばない。
...
Medium/High risk の場合は、概念モデル・責務表・境界/IF案・振る舞いテスト・手続き化リスクも含めてください。
...
（Structure-Behavior: Given / When / Then の振る舞いテスト）
