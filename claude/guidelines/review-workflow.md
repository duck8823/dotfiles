...
- Antigravity は共有デフォルトでは sandbox-first の `agy --print --sandbox` で走らせるが、無効化・sandbox・auth retry・write 可否は local policy を優先する
- Antigravity が quota・capacity・空出力で失敗した場合は、ブラウザ認証で止めずに Codex scout / default subagent へフォールバックする。ただし **認証プロンプト（login 失敗）の場合は fallback せず停止し、ユーザーに認証修正を依頼する**（暗黙の engine 代替は設定不備を隠す）。例外として `--sandbox` が host CLI の認証状態だけを隠した場合は、同一 engine / 同一 prompt / empty cwd / `NO_BROWSER=true` / no `--add-dir` / no `--sandbox` の authenticated transport retry を 1 回だけ行い、retry でも auth なら停止する
...
- Antigravity は共有デフォルトでは sandbox-first scout。local policy で無効化・auth retry・write 可否を上書き可。Codex verifier は reviewer config を使う
