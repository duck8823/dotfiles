...
- Antigravity は共有デフォルトでは `--approval-mode plan` で走らせるが、無効化・approval mode・write 可否は local policy を優先する
...
- Antigravity は共有デフォルトでは plan mode。local policy で無効化・上書き可。Codex verifier は reviewer config を使う
