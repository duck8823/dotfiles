...
- **model ID**: レビュー / 計画の本番レーンでは stable model の `gemini-2.5-pro` を明示する。`pro` / `auto` alias は preview access や `modelRouting` により `gemini-3-pro-preview` / `gemini-3.1-pro-preview` 側へ解決される可能性があるため、stable-only の品質ゲートでは alias を固定指定しない。preview 評価でだけ `gemini-3.1-pro-preview` 等を明示し、PR review / planning の通常 agent 定義へ混ぜない。
