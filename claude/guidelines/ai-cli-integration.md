# AI CLI 連携（Antigravity / Codex の呼び出し）
...
- **Antigravity** は policy-controlled scout / critic / optional worker として使う
- すべて **ヘッドレス実行** を基本とする
...
Claude / Antigravity / Codex の headless 調査は `~/.local/bin/multi-ai-research.sh`
...
### Antigravity
- 共有 dotfiles の既定は `agy --print --sandbox`（安全側）
...
- **model ID**: レビュー / 計画の本番レーンでは model は Antigravity CLI の routing に委ねる。固定が必要な場合だけ `MULTI_AI_ANTIGRAVITY_MODEL` / `--model` を使い、preview 評価を通常 review gate に混ぜない。
- **大入力で timeout**: 数千行規模の review は timeout しやすい。whole-repo を渡さず diff / sanitized packet を focused に保ち、per-run cwd と sandbox で入力範囲を絞る（Codex 同様、大 PR は grep 中心 + 代表ファイル spot-read の focused prompt にする）
...
### Antigravity（installed skills を使用）
```bash
  TERM=xterm-256color \
  agy --print --sandbox \
...
### Antigravity → current orchestrator / Claude

- Antigravity は local policy に従い、scout / critic / optional worker として使う
...
cmux を使って Codex / Antigravity を複数ペインで並列実行する。結果はファイル出力で回収し、Claude メインセッションが統合する。
...
$CMUX send --surface "$SURFACE_RIGHT" "  TERM=xterm-256color \
  agy --print --sandbox \
  < /tmp/antigravity-architect.md 2>&1 | tee /tmp/antigravity-architect-result.json && echo DONE"
...
PID_CODEX=$!
  ...
  TERM=xterm-256color \
  agy --print --sandbox \
  < /tmp/antigravity-architect.md 2>&1 | tee /tmp/antigravity-architect-result.json &
PID_ANTIGRAVITY=$!

wait $PID_CODEX $PID_ANTIGRAVITY
# pipefail 有効でも空出力のケースがあるため、ファイルサイズでも検証する
[[ -s /tmp/codex-architect-result.json ]] || echo "WARN: Codex result empty"
[[ -s /tmp/antigravity-architect-result.json ]] || echo "WARN: Antigravity result empty"
```
...
- 対話モード（`codex` / `agy`）は使わない

## エラーハンドリング

- Codex は stderr を別ファイルに保存する。Antigravity は `2>&1 | tee` で統合キャプチャする（応答テキストが stderr に混在するため）
