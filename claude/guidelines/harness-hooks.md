...
Traceary / Antigravity / Codex を含む横断 hook・observability 点検は
...
| 同上で `Antigravity` / `Claude` / `Codex` / `Gemini` の signature が PR コメントに無い | block。Multi-AI レビュー必須 |
| 同上で PR title/body の ticket 参照が 0 または複数 | block。1 PR = 1 ticket を満たすまで merge 不可 |
| AI local orchestrator の自律 merge | `duck8823` owner/org の PR のみ許可。その他 owner/org では具体的 PR の現ターンユーザー指示が必要 |
...
## Codex / Antigravity worktree ガード（`check-codex-worktree.sh`）

`codex exec` の read-only sandbox 以外、または raw shell の `agy --print --sandbox` 以外の Antigravity 実行を `main` / `master` ブランチで行おうとすると block。sandbox が auth を隠す場合の no-sandbox authenticated transport retry は、`multi-ai-research.sh` / review runner が empty cwd・NO_BROWSER・no `--add-dir`・同一 prompt で内部実行し、status に記録する。
...
block の発火は観測ログに記録される（`~/.claude/logs/stop-work-guard.log`、JSON Lines で `ts` / `session` / `turn_id` / `prompt_head`）。誤発火（委譲・質問での block）の有無を後から検証するために使う。ログ先は環境変数 `STOP_WORK_GUARD_LOG` で上書きでき、書き込み失敗時も block 判定は妨げない。
