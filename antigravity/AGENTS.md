...
- trusted repository での read-only / sandbox / plan の Claude / Antigravity / Codex / legacy Gemini 相互呼び出しは orchestrator 側 policy で default 許可（per-turn 明示依頼や ticket/PR 不要）。Antigravity は sandbox scout、legacy Gemini は read-only/plan/scout で応答し、write / commit / push / merge / deploy は自ら行わず委譲も受けない
