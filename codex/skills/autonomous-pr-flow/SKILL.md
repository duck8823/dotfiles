...
7. External AI delegation policy gate と local agent policy を確認し、許可される reviewer で `multi-ai-review` skill を実行する。policy deny / disabled / quota / timeout で失敗した場合は理由を記録して代替 reviewer を使う。login / 認証失敗（auth_prompt / ブラウザ認証プロンプト / 対話ログイン）の場合は別 engine へ暗黙に代替せず停止し、ユーザーに認証修正を依頼する
