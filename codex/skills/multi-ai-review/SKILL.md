...
この skill は `@claude @antigravity multi-ai-review` のような GitHub メンション方式を使わない。外部 AI を GitHub reviewer / collaborator として追加せず、ローカル CLI / subagent の結果を Codex が統合し、`gh pr comment` で PR に記録する。

## トリガー
- ユーザーが `multi-ai-review` を依頼したとき
- 自律PRフローで、PR作成後の多重レビューが必要なとき
- マージ前に Antigravity / Claude / Codex の観点を統合したいとき
...
- **調査失敗の分類**: Claude / Antigravity / Codex の workspace packet 調査や repo 外一次情報調査では `multi-ai-research` skill / `scripts/multi-ai-research.sh` を使い、`trust_failed` / `auth_prompt` / `quota_or_capacity` / `policy_or_permission_denied` / `prompt_file_reference_expansion` / `process_oom` / `timeout` / `command_failed` / `empty_output` を記録する。
...
|---|---|---|---|
| Codex | Antigravity/policy scout | Claude Code reviewer（不可なら default subagent reviewer） | Codex |
| Claude | Antigravity/policy scout | Codex verifier | Codex |
| Antigravity / 外部生成 | Codex verifier | Claude Code reviewer（不可なら default subagent reviewer） | Codex |
| 不明 | Antigravity/policy scout | Codex verifier + 必要なら default subagent reviewer | Codex |
...
ANTIGRAVITY_PREFLIGHT_OUT="$WORK_DIR/antigravity-preflight.md"
ANTIGRAVITY_REVIEW_OUT="$WORK_DIR/antigravity-review.md"

cat > "$ANTIGRAVITY_PROMPT_FILE" <<PROMPT
...
if [ -s "$POLICY_DENIED_FILE" ]; then
  ...
  echo "skipped: policy_denied: $(cat "$POLICY_DENIED_FILE")" > "$ANTIGRAVITY_REVIEW_OUT"
else
...
if [ "$ANTIGRAVITY_AVAILABLE" != true ]; then
  echo "Antigravity failed or unavailable. See $ANTIGRAVITY_PREFLIGHT_OUT / $ANTIGRAVITY_REVIEW_OUT" > "$ANTIGRAVITY_REVIEW_OUT"
fi
```

### 4. 独立 verifier

Codex authored PR では、可能なら Claude Code CLI を reviewer として使う。Claude Code CLI が使えない、quota、timeout の場合は default subagent reviewer へフォールバックし、欠落理由を記録する。ただし login / 認証失敗（auth_prompt / ブラウザ認証プロンプト / 対話ログイン）の場合は **default subagent reviewer へ暗黙に代替せず停止し、ユーザーに認証修正を依頼**する（設定不備を隠すため auth は fallback しない）。
...
Claude Code CLI を使う場合は、headless 実行が可能なことを `claude --help` 等で確認してから使う。対話ログインやブラウザ認証が出た場合は起動せず、**別 engine への暗黙の代替はせず停止し、ユーザーに認証修正を依頼**する（設定不備を隠すため auth は fallback しない）。

### 5. Codex 統合レビュー

Codex integrator は以下を行う。

1. Antigravity の repo-wide 指摘を読む。
...
| 失敗 | 対応 |
|---|---|
| Antigravity が browser auth prompt / 対話ログイン（login / 認証失敗） | ブラウザを開かず停止。暗黙の代替をせず、理由を記録してユーザーに認証修正を依頼 |
| Antigravity timeout / quota / 空出力 | 1回だけ再試行。失敗なら fallback |
| Claude Code CLI 不可（quota / timeout / 未導入） | default subagent reviewer または Codex verifier に fallbackし、利益相反リスクを明記 |
| Claude Code CLI が login / 認証失敗（auth_prompt / 対話ログイン） | 暗黙の代替をせず停止。理由を記録してユーザーに認証修正を依頼 |
...
- `@claude @antigravity multi-ai-review` を PR コメントに投げること。
