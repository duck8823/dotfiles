...
ユーザーが「自律的に進めて」「止まらず進めて」「このフローで回して」と指示した場合、`autonomous-pr-flow` を適用し、以下をデフォルト動作とする。
...
6. PR を Ready/Open にしてレビュー依頼
7. Codex レビューを待機（ポーリング）
8. 指摘をトリアージして反映
...
- レビュー待機中も止まらず、ポーリングで確認しながら並行可能な作業を進める
