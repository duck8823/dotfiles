...
- trusted repository での read-only/plan の Claude / Gemini / Codex 相互呼び出し（repository/workspace research・design consultation・triage・review planning）は default 許可で、per-turn の明示依頼や ticket/PR は不要。write/commit/push/PR 作成・merge/deploy/upload/destructive は default 対象外で、従来どおり 1 ticket / 1 PR scoped work（または許可された ticket-less prefix）に従う
