...
この文書は、Claude / Codex / Antigravity の skill・agent・subagent 定義を共通化するときの source of truth です。
各ツール固有の `CLAUDE.md`、`codex/agents/*.toml`、`antigravity/skills/*.md`、`SKILL.md` は、この役割表を投影したものとして扱います。
...
| Scout / critic | Antigravity / Claude / Codex | 既定は read-heavy。write は local policy 次第 | 既存パターン逸脱、命名 drift、diff 外影響 |
...
| Structure reviewer | Claude / Codex / Antigravity | read-heavy、必要なら設計メモ | 責務配置、境界/IF、振る舞いテスト不足 |
| Planner / spec writer | Codex / Antigravity / Claude | issue・コード調査 | wave、依存、担当、検証計画 |
...
6. **パーソナライズ情報はそのまま運用ルールにしない**
   Antigravity / Google Lens / 個人記憶から得た示唆は、出典・再現性・適用条件を確認してから dotfiles に蒸留する。スクリーンショットや個人データの raw dump は PR review / external AI prompt に含めない。

## 2026-05 時点の採用知見

- Claude / Antigravity とも subagent は「専門ロール + 独立 context + tool 制限」で安定する。万能 agent を増やすより、role / schema を共通化し、各 CLI の agent 定義へ薄く投影する。
...
## Claude / Antigravity / Codex research 協調
