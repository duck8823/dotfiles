...
Claude Code・Antigravity CLI・Codex CLI の設定ファイル一式。

AI エージェントを使った開発ワークフロー（計画・実装・レビュー・品質保証）を、**モデル名ベースではなく運転モードとローカルポリシーベース**で標準化するための設定です。

## 設計思想

この dotfiles では、各 AI を「どのモデルが強いか」ではなく、**どの運転モードに向くか**で使い分けます。オーケストレーターは固定称号ではなく、タスク・ローカルポリシー・可用性・その時点の能力で選ぶ role です。現在の実運用では Codex が主に担っていますが、Claude / Antigravity / Codex は同じ role schema で協調・代替できる前提にします。
...
| **Antigravity CLI** | Policy-controlled scout / critic / optional worker | 俯瞰レビュー・計画補助 | リポジトリ横断の一貫性確認、影響範囲洗い出し、マイルストーン計画、ドキュメント/設定更新漏れ検出 |

legacy Gemini は 4 つ目の explicit-only engine として残し、通常 routing には混ぜず、ユーザー明示または local policy で指定された read-only / plan / scout の範囲だけで使います。

### 基本方針

- **Orchestrator** は現在の main session が担う。いまは Codex が多いが、Claude / Antigravity がより適する局面では切り替えられるよう role / schema / policy を共通化する
- **Claude / Antigravity** は固定された上下関係ではなく、専門 agent / reviewer / scout / worker として協調する
- **Antigravity を read-only に固定しない**。dotfiles の共有テンプレートでは安全側の sandbox-first scout 設定を置くが、write 可否・無効化・モデル・sandbox・auth retry はローカルポリシーで上書きする
- ベンチマークではなく、**失敗率・再現性・レビュー品質・割り込み耐性**で運用判断する

## 前提条件

- [Claude Code](https://claude.ai/code) がインストール済み
- [Antigravity CLI](https://antigravity.google/docs/gcli-migration) がインストール済み（`agy` コマンドが使えること）
...
`~/.claude/settings.json` / `~/.gemini/antigravity-cli/settings.json` / `~/.codex/config.toml` のような**ユーザー設定ファイル**は、インストーラが内容ハッシュを追跡します。
...
| `/multi-ai-research <topic>` | Claude / Antigravity / Codex に同一 context packet で調査させる |

#### `/sprint` の特徴

- 実装前に **Codex + Antigravity の 2系統スカウト**を実施
  - Codex: テスト戦略・セキュリティ・実装分割
  - Antigravity: 既存パターン・命名一貫性・diff 外影響
...
| レビュアー | 観点 |
|-----------|------|
| Antigravity CLI | repo-wide 一貫性、パターン逸脱、diff 外影響、ドキュメント/設定漏れ |
...
Antigravity / legacy Gemini / Codex / Claude CLI への delegation は `~/.codex/config.toml` の `[auto_review].policy` にある **External AI delegation exception** を満たす場合に実行します。

- 原則は trusted repository / git worktree 上で、1 ticket / 1 PR に限定
- trusted repository での read-only / sandbox / plan の Claude / Antigravity / Codex / legacy Gemini 相互呼び出し（repository/workspace research・design consultation・triage・review planning）は default 許可。write / commit / push / PR 作成 / merge / deploy / upload / destructive は対象外で、従来どおり scoped work と forbidden actions に従う
- structured-output regeneration / AI-reviewed generated artifact update は、version-controlled な sample/prompt/schema/dictionary/generator と sanitized context に入力を限定し、外部AIへ direct write/commit/push を委譲せず local validation を必須にする場合だけ ticket/PR 不要
- 例外として、ユーザーが現在ターンで明示的に Claude / Antigravity / Codex / multi-AI research を求め、public/general Web 調査だけを行う場合は、ローカルファイル・source code・workspace packet・shell history・credentials・tokens・private data を送らない read-only/headless 調査として許可
...
- Path C（trusted creative / cross-agent collaboration）は、現スレッドで依頼済みの 1 coherent workstream に限定して、選定済み画像・PR diff・必要最小限の関連ソース・prompt / style guide / 検証ログの共有を許可する。未選別の Google Photos / browser profile / cookies / secrets / credentials / unrelated home・repo dump は共有禁止
- Path C でも外部 AI へ direct write / commit / push / PR 作成 / merge / deploy / infra apply / store・TestFlight upload は委譲しない。共有した画像・prompt は path と sha256、engine classification、local validation、残リスクを記録する
- PR diff / local branch diff / workspace context packet / 関連ソース / テスト出力など必要最小限だけを渡す。general Web 調査では current user request、非機密の短い project summary、public URL、出力 schema だけに限定する
...
- 外部 AI CLI は host 側の既存認証（CLI login / keychain / `CODEX_HOME` 等）を使える surface で起動し、データ境界は sanitized packet・empty cwd・NO_BROWSER・no broad `--add-dir`・read-only/plan/scout 権限で作る。sandbox を auth 境界として使わない
- Antigravity `--sandbox` が host CLI の認証状態だけを隠して `auth_prompt` になった場合は、同一 engine / 同一 prompt / empty cwd / NO_BROWSER / no `--add-dir` / no `--sandbox` の `authenticated_transport_without_cli_sandbox` retry を 1 回だけ許可し、両 attempt を記録する。retry でも auth なら fallback せず停止する
...
- 共有テンプレートの Antigravity は安全側の sandbox-first を既定にするが、Antigravity の無効化・write 許可・sandbox・auth retry はローカルポリシーで上書きできる。Codex verifier は reviewer config を優先する
...
このスキルは、AI が要件から手続き的実装へ直行するリスクを抑えつつ、Claude / Codex / Antigravity の役割分担に合わせて軽量に使えるよう調整しています。

### Antigravity 設定（`~/.gemini/antigravity-cli/`）

| ファイル | 用途 |
|--------|------|
| `AGENTS.md` | Antigravity 全体のグローバル運用方針 |
| `skills/*/SKILL.md` | planner / reviewer / structure-reviewer などの scout / critic スキル |
| `settings.json` | sandbox / request-review を既定にした Antigravity CLI 設定。ローカル編集で上書き可 |
...
| `check-codex-worktree.sh` | PreToolUse | `codex exec` / `agy`（write モード）を main / master ブランチで実行することをブロック |
...
共有 dotfiles は安全側のデフォルトを置くだけで、各 agent の最終的な使い方はローカルポリシーで決めます。`~/.config/ai-agent-policy.env` または環境変数で、Antigravity を無効化したり、multi-AI 調査で使う engine を絞れます。

```bash
# 例: Antigravity をこのマシンでは使わない
MULTI_AI_ENGINES=claude,codex
MULTI_AI_DISABLED_ENGINES=antigravity

# 例: Antigravity の headless review は sandbox-first。sandbox が auth だけを隠す場合は同一 engine retry
...
`~/.gemini/antigravity-cli/settings.json` は installer が内容ハッシュを追跡します。
dotfiles 側の更新とローカル編集が衝突した場合は `~/.gemini/antigravity-cli/settings.json.dotfiles-new` を確認してください。
...
- `~/.gemini/antigravity-cli/settings.json.dotfiles-new`
