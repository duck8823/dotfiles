...
この dotfiles では、各 AI を「どのモデルが強いか」ではなく、**どの運転モードに向くか**で使い分けます。現在の標準は **Codex をメインオーケストレーター**に置き、Claude / Gemini はタスク・ローカルポリシー・可用性に応じて協調させます。

| エージェント | 標準モード | 主担当 | 向いている仕事 |
|---|---|---|---|
| **Codex CLI** | Primary orchestrator / worker / verifier | 全体進行・実装・検証・セキュリティ | Issue/PR の自律進行、スコープが明確な実装、テスト追加、CI/CD、シェル、自動検証、レビュー反映 |
| **Claude Code** | Foreground specialist / integrator | UX・仕様判断・大きめの統合判断 | ユーザー体験を変える判断、複数レイヤー統合、最終 diff の人間向け説明、Claude Code 固有の作業 |
| **Gemini CLI** | Policy-controlled scout / critic / optional worker | 俯瞰レビュー・計画補助 | リポジトリ横断の一貫性確認、影響範囲洗い出し、マイルストーン計画、ドキュメント/設定更新漏れ検出 |

### 基本方針

- **Codex** は現在のメイン操縦席。Traceary / git / PR / workspace packet から context を復元して継続する
...
- Codex 主体の運用では Codex が進行し、Claude / Gemini は必要な観点の reviewer / specialist として参加する
...
Codex 主体の再開では `codex/skills/context-resume` を使い、手書きの Claude→Codex 引き継ぎではなく、Traceary handoff / recent context / git status / PR / Issue から objective・scope・検証状態を復元します。
