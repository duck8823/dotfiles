...
- `pnpm` がインストール済み（memories.sh のインストールに使用。なければスキップ）
...
| `hooks.json` | Codex CLI のフック設定 |
| `hooks/memories-session.sh` | memories.sh セッションライフサイクル管理 |
...
| `settings.json` | read-only scout 運用向けのユーザー設定（テンプレート生成） |
| `hooks/memories-session.sh` | memories.sh セッションライフサイクル管理 |
...
| `memories-session.sh` | SessionStart / Stop | memories.sh セッションの開始・snapshot・終了 |
