...
- **Antigravity を read-only に固定しない**。dotfiles の共有テンプレートでは安全側の scout 設定を置くが、write 可否・無効化・モデル・sandbox はローカルポリシーで上書きする
...
- 共有テンプレートの Antigravity は安全側の sandbox を既定にするが、Antigravity の無効化・write 許可・sandbox はローカルポリシーで上書きできる。Codex verifier は reviewer config を優先する
...
# 例: Antigravity の headless review は sandbox に固定
